...
Update generated marketplace indexes so [BPG](https://github.com/ginstrom/bpg) can discover recipes and gather the metadata needed for build-time execution plan generation.

## Scope

This step changes index generation output. It does not implement [BPG](https://github.com/ginstrom/bpg) execution plans inside this repository.
...
Generated indexes should let a [BPG](https://github.com/ginstrom/bpg) build flow answer:
...
- Declarative mapping transforms remain visible to [BPG](https://github.com/ginstrom/bpg) plan generation.
...
Resolution should still happen in [BPG](https://github.com/ginstrom/bpg). This repository should provide enough indexed metadata for [BPG](https://github.com/ginstrom/bpg) to generate a locked plan with exact package versions, image digests, node IDs, node versions, schema versions, defaults, and workflow bindings.

Keep `generated/*.json` ignored unless this repository becomes the direct distribution surface for [BPG](https://github.com/ginstrom/bpg) clients. CI should still build the files on every change so schema, registry, and indexer drift is caught before merge.
